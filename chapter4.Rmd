...
The elbow plot shows that the last point with decreased WCSS is 9, so I decided to look what k=9 clustering looks like.
...
Boston_scaled_km <- kmeans(Boston_scaled, centers = "9")
```
...
lda.arrows(lda.fit, myscale = 3.5)
```
As we can see, the cluster 2 is saparate from all the othersm and the Charles River variable is the main determinant for this cluster. 
Clusters 9 and 1 are separate from others, this separation is dependant on the radial roads variable, cluster 6 seems to be seaparated form the mass, and zn is the main contributing factor.
...
The crime colouring shows that high crime group is clustered separately, with some med-high groups. The k-means clustering shows clusters 1 and 9 to be together, as in 2d plain, however the second k-cluster is split between the two large clusters.
